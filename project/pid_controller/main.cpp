...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  double steer_p = 0.1; // 0.2;
  double steer_i = 0.004; // 0.004;
  double steer_d = 0.3;// 0.3; 
  ...
  **/
  ...
  double throttle_p = 0.01;// 0.005;
  double throttle_i = 0.00005;//0.0002; 
  double throttle_d = 0.3;// 0.15;
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          // The error is the angle difference between the desired yaw (angle_between_points(x_points[x_points.size() -2], y_points[y_points.size() -2], x_points[x_points.size() -1], y_points[y_points.size() -1])) and the actual yaw (yaw) 
          //error_steer = angle_between_points(x_points[0], y_points[0], x_points[1], y_points[1]) - yaw;
          ...
          **/
          ...
          // The error is the speed difference between the desired velocity (last point of v_points) and the current speed (velocity)
